...
def test_registration_with_defaults_works():
    ...
    builder.services.add_factory(
        ...
        lambda p: NotificationService(
            email_service=None,  # Would normally inject this
            app_name="TestApp"
        )
    )
...
def test_mixed_primitive_and_complex_dependencies():
    ...
    class ConfigurableNotificationService:
        def __init__(self, email_service: IEmailService, prefix: str = "[NOTIFICATION]"):
            self.email_service = email_service
            self.prefix = prefix
    ...
    builder.services.add_factory(
        IEmailService,
        lambda p: EmailService("smtp.test.com", 25, False)
    )
