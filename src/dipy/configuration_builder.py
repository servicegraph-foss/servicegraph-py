...
class ConfigurationBuilder(IConfigurationBuilder):
    ...
    def add_json_file(self, path: Union[str, Path], optional: bool = False) -> 'ConfigurationBuilder':
    ...
    def add_environment_variables(self, prefix: str = "") -> 'ConfigurationBuilder':
        ...
        for key, value in os.environ.items():
            if not prefix or key.startswith(prefix):
                ...
                if '__' in config_key:
                    self._set_nested_value(env_data, config_key.replace('__', ':'), value)
                else:
    ...
    def _deep_merge(self, target: Dict[str, Any], source: Dict[str, Any]) -> None:
        ...
        for key, value in source.items():
            ...
            if matching_key and isinstance(target[matching_key], dict) and isinstance(value, dict):
