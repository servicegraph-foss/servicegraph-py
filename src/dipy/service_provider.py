    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if hasattr(self._service_instance, 'dispose'):
            try:
                ...
                print(f"Error disposing scoped service {type(self._service_instance).__name__}: {e}")
        elif hasattr(self._service_instance, 'close'):
            try:
                self._service_instance.close()
            except Exception as e:
                print(f"Error closing scoped service {type(self._service_instance).__name__}: {e}")
        ...
        if self._session_id in self._provider._scoped_instances:
            ...
            for scoped_dep in scoped_deps:
                if scoped_dep is not self._service_instance:  # Don't double-dispose the main instance
                    if hasattr(scoped_dep, 'dispose'):
                        try:
                            scoped_dep.dispose()
                        except Exception as e:
                            print(f"Error disposing scoped dependency {type(scoped_dep).__name__}: {e}")
                    elif hasattr(scoped_dep, 'close'):
                        try:
                            scoped_dep.close()
                        except Exception as e:
                            print(f"Error closing scoped dependency {type(scoped_dep).__name__}: {e}")
...
class ServiceProvider:
    ...
    def __init__(self, service_collection: ServiceCollection):
        ...
        self._scoped_instances: Dict[str, List[Any]] = {}  # session_id -> list of scoped instances
    ...
    def get_service(self, service_type: Type[T], session_id: Optional[str] = None) -> T:
        ...
        if registration_key not in self._collection._registrations:
            ...
            similar_services = [svc for svc in available_services if service_type.__name__.lower() in svc.lower()]
    ...
    def get_named_service(self, service_type: Type[T], name: str, session_id: Optional[str] = None) -> T:
        """
        ...
        if registration_key not in self._collection._registrations:
            ...
            if available_names:
                ...
                if base_key in self._collection._registrations:
                    ...
                    error_msg += f"Use: provider.get_service({service_type.__name__}) instead."
                else:
    ...
    def get_all_named_services(self, service_type: Type[T], session_id: Optional[str] = None) -> Dict[str, T]:
        """
        ...
        for registration in self._collection._registrations.values():
            if registration.service_type == service_type and registration.is_named:
                result[registration.name] = self._get_or_create_instance(registration, session_id)
    ...
    def get_all_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        for registration in self._collection._registrations.values():
            ...
            is_factory = registration.implementation != registration.service_type or hasattr(registration.factory, '__name__')
            
            # For factory registrations, try to get a more descriptive name
            impl_name = registration.implementation.__name__ if registration.implementation else 'Factory'
            ...
            services[service_key] = {
                ...
                'is_instance': 'instance_factory' in str(registration.factory) if registration.factory else False
            }
        
        return services
    
    def get_services_by_type(self, service_type: Type = None) -> Dict[str, Dict[str, Any]]:
        """
        ...
        return {
            key: info for key, info in all_services.items()
    ...
    def remove_all_by_type(self, service_type: Type) -> int:
        ...
        keys_to_clear = [
            key for key, reg in self._collection._registrations.items()
    ...
    def _get_or_create_instance(self, registration, session_id: Optional[str] = None, is_dependency: bool = False) -> Any:
        """
        ...
        if registration.lifetime == ServiceLifetime.TRANSIENT:
            ...
            if effective_session_id:
                return self._get_or_create_session_service(registration, effective_session_id)
            else:
            ...
            if registration.registration_key not in self._singleton_instances:
                with self._lock:
                    if registration.registration_key not in self._singleton_instances:
                        self._singleton_instances[registration.registration_key] = registration.factory(self)
    ...
    def _get_or_create_session_service(self, registration, session_id: str) -> Any:
        ...
        with self._instance_lock:
            ...
            if registration.registration_key not in session_services:
                session_services[registration.registration_key] = registration.factory(self)
